...
from dltrain.data import flatten_and_index, formatea_y_codifica_normales
...
def run():
    ...
    # np_dataset_pts_lst: LISTA CON LOS PUNTOS DE LOS LANDMARKS DE CADA ARCHIVO
    # np_dataset_visibility_list: LISTA CON LA VISIBILIDAD DE LOS LANDMARKS DE CADA ARCHIVO
    filenames, np_dataset_visibility_list = read_visiblity_lst(args)
    filenames_repetidos, np_dataset_pts_lst = read_pts_lst(args)
    ...
    def train_joint_model(model, tr_dl, ts_dl, epochs, label='dl training', lr=lr):
        ...
        for e in pbar:
            ...
            for k in keys:
                ...
                for minibatch in dl:
                    ...
                    mini_im, mini_normal, mini_lm, mini_ohe = formatea_y_codifica_normales(*minibatch)
                    model.zero_grad()	
                    mini_input = torch.cat((mini_ohe,mini_normal),dim=1)

                    mini_im = mini_im.to(device)
                    mini_lm = mini_lm.to(device)
                    mini_input = mini_input.to(device)

                    with torch.set_grad_enabled(is_train):
                        mini_pred = model(mini_im, mini_input)
        ...
        if TEST_KEY in keys:
            with torch.no_grad():
                mini_im, mini_normal, mini_lm, mini_ohe = formatea_y_codifica_normales(*next(iter(dl_dict[TEST_KEY])))
                mini_input = torch.cat((mini_ohe,mini_normal),dim=1)
                ...
                last_pred = model(mini_im, mini_input).detach().cpu().view((-1, len(LM_NAMES), 2))
        else:
            last_pred = None

        return loss_history, last_pred, acc_history
    ...
    def joint_training(yp_all, tr_idx, ts_idx, q_maxerror, cv_idx, normales,**kwargs):
        ...
        tr_ds = dataset_class(path_list=[os.path.join(FORENSIC_DS, x) for x in filenames[tr_idx]],
                              center_list=yp_tr, visible_list=np_dataset_visibility_list[tr_idx],
        ...
        if ts_idx is not None:
            ts_ds = dataset_class(path_list=[os.path.join(FORENSIC_DS, x) for x in filenames[ts_idx]],
                                  center_list=yp_ts, visible_list=np_dataset_visibility_list[ts_idx],
        ...
        train_history, lm_yp_ts, acc_history = train_joint_model(model, tr_dl, ts_dl, epochs,
        ...
        plot_history(train_history, epochs, title=EXPERIMENT_KEY).savefig(os.path.join(out_path, model_key + '.jpg'))
        plot_accuracy(acc_history, epochs, title=EXPERIMENT_KEY).savefig(os.path.join(out_path, model_key + "_acc.jpg"))
        plt.close()
    ...
    def get_3ddfa_prediction(fold_idx, tr_idx) -> Tuple[np.ndarray, Dict]:
        ...
        normales_lst = np.load("normales_lst.npy")
        return yp_all, {'best_idx': best_idx}, normales_lst[:,:,best_idx]



    # Initialize prediction
    yp = np.zeros((len(np_dataset_visibility_list), 30, 2)) + np.nan
    errores_folds = []

    
    for cv_idx, (tr_idx, ts_idx) in enumerate(tqdm(kfold_idxs)):
        # PREDICCIONES BASADAS EN 3DDFA
        yp_all, extra_info, normales = get_3ddfa_prediction(cv_idx, tr_idx)
        ...
        yp[ts_idx], error_idx = joint_training(yp_all,tr_idx, ts_idx, q_maxerror, cv_idx,normales)
    ...
    for idx_imagen in range(len(ts_idx)):
        for landmark in range(30):
             ...
             pred = np.argmax(yp[img][landmark])
    ...
    for dato in yp:
        for pred in dato:
            salida = salida + str(pred[0]) + " " + str(pred[1]) + "\n"
    '''
